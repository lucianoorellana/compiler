...
compileEnv env (Tuple es l)      = tupleReserve (annTag l) ((4 * roundUp (((length es)) + 2))) ++ 
  ...
  where
    ...
    tupleCopy (e,n)              = [ IMov (Reg EBX) (immArg env e)
                                    , IMov (addr (n+2)) (Reg EBX)]
    roundUp n                    = if (n `mod` 2 == 0) then n else n + 1
    addr n                       = Sized DWordPtr (RegOffset (4*n) EAX)
